...
class TrashCanController(ControllerBase):
    ...
    def login_app(self):
        try:
            login_request = requests.post(
                "http://192.168.12.1/login_app.cgi",
                data={"name": self.username, "pswd": self.password},
            )
        except:
    ...
    def get_site_info(self):
        try:
            ...
            stat_request = requests.get(
                "http://192.168.12.1/cell_status_app.cgi", cookies=self.app_jar
            )
        except:
        ...
        return {"eNBID": int(meta["eNBID"]), "PLMN": meta["MCC"] + "-" + meta["MNC"]}
    ...
    def login_web(self):
        ...
        try:
            login_request = requests.post(
                "http://192.168.12.1/login_web_app.cgi", data=login_request_body
            )
        except:
            logging.critical("Could not post login request, exiting.")
            sys.exit(ExitStatus.API_ERROR.value)
    ...
    def reboot(self):
        try:
            if not (self.csrf_token or self.web_jar):
                self.login_web()
            reboot_request = requests.post(
                "http://192.168.12.1/reboot_web_app.cgi",
                data={"csrf_token": self.csrf_token},
                cookies=self.web_jar,
            )
        except:
    ...
    def get_device_info(self):
        try:
            device_info_req = requests.get(
                "http://192.168.12.1/dashboard_device_info_status_web_app.cgi"
            )
        except:
